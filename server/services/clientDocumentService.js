 ...
 */
export function expiryStatus (expiryDate, withinDays = 30) {
 ...
 */
export function updateClientDocument (clientId, id, meta = {}) {
 ...
 * @returns {object[]}
 */
export function listExpiringDocuments (withinDays = 90) {
  const soon = new Date(Date.now() + withinDays * 24 * 60 * 60 * 1000).toISOString().slice(0, 10)
...
export function countExpiringDocuments (withinDays = 90) {
  const soon = new Date(Date.now() + withinDays * 24 * 60 * 60 * 1000).toISOString().slice(0, 10)
 ...
 * @returns {object}
 */
export function restoreClientDocument (id) {
