...
import { clientDisplayName, getClient } from './clientService.js'
import { getShift } from './shiftService.js'
 ...
 */
export function listIncidents (pg, filters = {}) {
 ...
 * @returns {object}
 */
export function createIncident (data, workerId) {
 ...
 * @returns {object}
 */
export function createFromShift (shiftId, workerId) {
...
export function countUnreportedReportable () {
...
export const incidentTypeLabel = t => TYPE_LABELS[t] || t
 ...
 * @returns {string}
 */
export function buildIncidentMarkdown (incident) {
  const i = incident
  const yn = v => (v ? 'Yes' : 'No')
