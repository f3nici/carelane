...
/**
 * Build the deleted-items (recycle bin) service bound to a host context. It
 * dispatches restores to every other service, so it is assembled last.
 * @param {import('./index.js').CoreContext} ctx
 * @param {object} services assembled core services
 */
export function createDeletedService (ctx, services) {
  const { sqlite } = ctx
  const { clientDisplayName, restoreClient } = services.client
  const { restoreAgreement } = services.agreement
  const { restoreShift } = services.shift
  const { restoreScheduled } = services.schedule
  const { restoreReport } = services.report
  const { restoreTemplate } = services.template
  const { reactivateBillingCode } = services.billing
  const { restoreClientDocument } = services.clientDocument
  const { restoreGoal } = services.goal
  const { restoreIncident, incidentTypeLabel } = services.incident
  const { restoreRestrictivePractice } = services.restrictivePractice
  const { restoreMedicationRecord } = services.medication
   ...
   */
  function restoreDeleted (type, id) {
