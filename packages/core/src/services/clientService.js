...
/**
 * Build the participant (client) service bound to a host context.
 * @param {import('./index.js').CoreContext} ctx
 * @param {object} services assembled core services (for crypto)
 */
export function createClientService (ctx, services) {
  const { sqlite } = ctx
  const { encrypt, decrypt, decryptFields, blindIndex } = services.crypto
   ...
   */
  function buildClientExportMarkdown (data) {
    const c = data.client
    const lines = []
   ...
   */
  function getClientBillingCodes (clientId) {
    getClient(clientId)
   ...
   */
  function setClientBillingCodes (clientId, codes) {
    getClient(clientId)
