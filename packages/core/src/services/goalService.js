...
/**
 * Build the structured-goals service bound to a host context.
 * @param {import('./index.js').CoreContext} ctx
 * @param {object} services assembled core services
 */
export function createGoalService (ctx, services) {
  const { sqlite } = ctx
  ...
  const GOAL_STATUSES = new Set(['active', 'achieved', 'on_hold', 'discontinued'])
   ...
   * @returns {object[]}
   */
  function listGoals (clientId, filters = {}) {
   ...
   * @returns {object}
   */
  function createGoal (clientId, data) {
    getClient(clientId)
    const ts = now()
   ...
   * @returns {object}
   */
  function updateGoal (clientId, id, data) {
    getGoal(clientId, id)
    const sets = []
    const params = []
   ...
   * @param {number} id
   */
  function deleteGoal (clientId, id) {
    getGoal(clientId, id)
   ...
   */
  function addProgressNote (clientId, goalId, data) {
   ...
   * @returns {object} the updated goal (with progress)
   */
  function deleteProgressNote (clientId, goalId, noteId) {
    getGoal(clientId, goalId)
   ...
   * @returns {string|null}
   */
  function buildGoalsSummary (clientId, perGoalNotes = 3) {
