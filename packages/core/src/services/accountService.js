 ...
 * (cost 12, matching the seed) — they are never reversibly encrypted. bcryptjs
 * is pure JS, so this runs on both hosts.
 * @param {import('./index.js').CoreContext} ctx
 */
export function createAccountService (ctx) {
  const { sqlite } = ctx
  ...
  return { changePassword, destroyOtherSessions, setPasswordByUsername }
}
