...
/**
 * Build the billing-code service bound to a host context.
 * @param {import('./index.js').CoreContext} ctx
 */
export function createBillingService (ctx) {
  const { sqlite } = ctx
   ...
   */
  function listBillingCodes (pg, filters = {}) {
    const where = ['1=1']
    const params = []
   ...
   */
  function createBillingCode (data) {
    const ts = now()
   ...
   * @param {object} data
   */
  function updateBillingCode (id, data) {
    getBillingCode(id)
   ...
   * @param {number} id
   */
  function deactivateBillingCode (id) {
    getBillingCode(id)
   ...
   * @param {number} id
   */
  function reactivateBillingCode (id) {
    getBillingCode(id)
