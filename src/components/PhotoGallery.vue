...
// photos are served only via this auth-gated endpoint, never a static path
const photoUrl = id => `/api/v1/shifts/${props.shiftId}/photos/${id}/file`
...
<template>
  <div v-if="photos.length" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
    <figure v-for="p in photos" :key="p.id" class="relative group rounded-xl overflow-hidden border border-white/10">
      <img :src="photoUrl(p.id)" :alt="p.caption || p.original_name" class="h-32 w-full object-cover" loading="lazy" />
  ...
  <p v-else class="text-sm text-mid">No photos attached.</p>
</template>
